...
Telegraph::editMedia($messageId)->videoNote($path)->send();
...
you can also pass an array of options if you wish
...
set custom Telegraph business connection id

```php
Telegraph::message('test')->inBusiness(business_connection_id);
```
