...
```php
Telegraph::readBusinessMessage($messageId)->inBusiness($business_connection_id)->send();
```
...
```php
Telegraph::deleteBusinessMessages($messageIds)->inBusiness($business_connection_id)->send();
```
...
removes a message keyboard (see [keyboards](../12.features/3.keyboards) for details)
...
replace a message keyboard (see [keyboards](../12.features/3.keyboards) for details)
