...
[TelegraphBots](../14.models/1.telegraph-bot) data storage is available through its `->storage()` method
...
[TelegraphChats](../14.models/2.telegraph-chat) data storage is available through its `->storage()` method
...
Depending on the application needs, multiple drivers can be [configured](../2.installation#configuration) and used.
...
A file storage driver saves data in json files inside the disk/folder defined in Telegraph [configuration file](../2.installation#configuration)
...
A cache storage driver saves data using Laravel Cache system following Telegraph [configuration file](../2.installation#configuration)
