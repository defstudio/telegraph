...
if you are implementing a custom bot management logic, you can register a webhook using the `TelegraphBot` model:
...
> [!WARNING]
> Manual updates polling is not available if a webhook is set up for the bot. The webhook should be removed first using its [unregisterWebhook](3.deleting-webhooks#programmatically) method
