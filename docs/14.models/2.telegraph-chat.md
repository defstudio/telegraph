...
Retrieves the Endpoint Url. Optionally you can configure a custom Endpoint Url (see [Custom Url](models/telegraph-bot)

```php
/** @var \DefStudio\Telegraph\Models\TelegraphChat $telegraphChat */

$telegraphChat->getUrl();
```

### `getFilesUrl()`

Retrieves the Files Url. Optionally you can configure a custom Endpoint Url (see [Custom Url](models/telegraph-bot)

```php
/** @var \DefStudio\Telegraph\Models\TelegraphChat $telegraphChat */

$telegraphChat->getFilesUrl();
```
...
Attachments can be sent to a business chat through Telegraph `->inBusiness()` method:

```php
$telegraphChat->message('hi')->inBusiness(BUSINESS_ID)->send();
```
...
$telegraphChat->editMedia($messageId)->videoNote($path)->send();
$telegraphChat->editMedia($messageId)->audio($path)->send();
```

### `replaceKeyboard()`

Starts a `Telegraph` call to replace a message keyboard (see [keyboards](../12.features/3.keyboards) for details)
...
Starts a `Telegraph` call to remove a message keyboard (see [keyboards](../12.features/3.keyboards) for details)
...
$telegraphChat->readBusinessMessage($messageId)->inBusiness($business_connection_id)->send();
```
...
$telegraphChat->deleteBusinessMessages($messageIds)->inBusiness($business_connection_id)->send();
```
...
$telegraphChat->setHttpProxy('http://user:password@host:port')->message('message through proxy')->send();
```
...
sends an invoice. More on [invoice](../12.features/7.attachments#invoice)
...
sends a game. More on [game](../12.features/7.attachments#game)

```php
$telegraphChat->game('game_short_name')->send();
```

### `richMessage()`

sends a rich message. More on [Rich Message](../12.features/7.attachments#rich-message)

```php
$telegraphChat->richMessage('<ul>Rich Message</ul>')->send();
```
...
/** @var DefStudio\Telegraph\Models\TelegraphChat $telegraphChat */
$telegraphChat->quiz("What's your favourite programming language?")
    ->options(['php' => true, 'typescript', 'rust'])
    ->explanation('We all love php, right?')
    ->validUntil(now()->addMinutes(5))
    ->send();
```
...
A video note attachment can be sent through Telegraph `->videoNote()` method:

```php
/** @var DefStudio\Telegraph\Models\TelegraphChat $telegraphChat */
$telegraphChat->videoNote()->send();
```
