...
Attachments can be sent to a specific Thread through Telegraph `->thread()` method:

```php
$telegraphChat->message('hi')->thread('thread_id')->send();
$telegraphChat->thread('thread_id')->message('hi')->send();
```
