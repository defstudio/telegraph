...
End-to-end пример для webhook command смотрите в first-bot testing path: [Build your first Telegram bot with Telegraph](../11.quickstart/6.build-your-first-bot#6-test-the-handler). Он сочетает `Telegraph::fake()`, `Telegraph::assertSent()` и `Telegraph::assertSentData()`, чтобы проверить, что `/start` отвечает в ожидаемый first chat.
