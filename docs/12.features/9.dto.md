...
- `->isRequest()` true, if the access was granted after the user accepted an explicit request from a Web App sent by the method [requestWriteAccess](https://core.telegram.org/bots/webapps#initializing-mini-apps)
- `->isWebApp()` a helper method that returns true if the access was granted when the Web App was launched from a link
- `->webAppName()` Name of the Web App, if the access was granted when the Web App was launched from a link
- `->isWebApp()` true, if the access was granted when the bot was added to the attachment or side menu
