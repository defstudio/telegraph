...
```php
Telegraph::message('hi')->inBusiness(BUSINESS_ID)->send();
```
...
and validation checks limits can be customized in [telegraph.php config](../2.installation#Configuration) file (`attachments` section)
...
More on [invoice](11.payments#Attachments)
...
More on [game](10.games#Attachments)
...
```php
Telegraph::richMessage('<ul>Rich Message</ul>')
        ->send();
```

More on [Rich Message](12.rich-messages#Attachments)
...
> [!WARNING]
> Sent Photos can be edited with the [editMedia](../13.api/3.messages#editMedia) call
...
> [!WARNING]
> Sent Animations can be edited with the [editMedia](../13.api/3.messages#editMedia) call
...
> [!WARNING]
> Sent Videos can be edited with the [editMedia](../13.api/3.messages#editMedia) call

### VideoNote

Rounded square video can be sent through Telegraph `->videoNote()` method:

```php
Telegraph::videoNote(Storage::path('video.mp4'))->send();
Telegraph::videoNote('https://my-repository/video.mp4')->send();
Telegraph::videoNote($telegramFileId)->send();
```

> [!WARNING]
> Sent Video Notes can be edited with the [editMedia](../13.api/3.messages#editMedia) call
...
> [!WARNING]
> Sent Audio messages can be edited with the [editMedia](../13.api/3.messages#editMedia) call
...
> [!WARNING]
> Sent Documents can be edited with the [editMedia](../13.api/3.messages#editMedia) call
...
> [!WARNING]
> Sent attachment captions can be edited with the [editCaption](../13.api/3.messages#editCaption) call
